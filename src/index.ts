...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Meteors');
}
...
async function getFireballs(limit: number) {
  ...
  const res = await pwFetch(`${JPL_BASE}/fireball.api?${params}`);
  if (!res.ok) throw await httpError(res, 'JPL Fireball API error');
...
async function getCloseApproaches(limit: number) {
  ...
  const res = await pwFetch(`${JPL_BASE}/cad.api?${params}`);
  if (!res.ok) throw await httpError(res, 'JPL CAD API error');
...
async function getNeoFeed(startDate: string, endDate: string) {
  ...
  const res = await pwFetch(`${NEO_BASE}/feed?${params}`);
  if (!res.ok) throw await httpError(res, 'NASA NeoWs API error');
